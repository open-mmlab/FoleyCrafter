...
def args_parse():
    ...
    config.add_argument('--pretrain', type=str, default='auffusion/auffusion-full-no-adapter', help='audio generator pretrained checkpoint path')
...
def build_models(config):
    ...
    vocoder            = Generator.from_pretrained(
                        vocoder_config_path,
                        subfolder="vocoder").to(config.device)
    ...
    pipe.load_ip_adapter('./models/semantic', subfolder='', weight_name='semantic_adapter.bin', image_encoder_folder=None)
...
def run_inference(config, pipe, vocoder, time_detector):
    ...
    image_encoder        = CLIPVisionModelWithProjection.from_pretrained('h94/IP-Adapter', subfolder='models/image_encoder').to(config.device)
    input_list.sort()
    with torch.no_grad():
        for input_video in input_list:
            ...
            time_condition = [-1 if preds[0][int(i / (1024 / 10 * duration) * 150)] < 0.5 else 1 for i in range(int(1024 / 10 * duration))]
            time_condition = time_condition + [-1] * (1024 - len(time_condition))
            # w -> b c h w
            time_condition = torch.FloatTensor(time_condition).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(1, 1, 256, 1).to('cuda')
            ...
            video_save_path = osp.join(config.save_dir, f'video')
